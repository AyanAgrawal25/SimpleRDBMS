...
class BufferManager{
    ...
    Page* insertTablePageIntoPool(string tableName, uint runsThisPass, int pageIndex);
    Page* insertMatrixPageIntoPool(string tableName, int pageIndex);
    public:
    
    BufferManager();
    Page* getPage(string tableName, int pageIndex);
    Page getTablePage(string tableName, int pageIndex);
    Page getTablePage(string tableName, uint runsThisPass, int pageIndex);
    MatrixPage getMatrixPage(string matrixName, int pageIndex);
    void writePage(string pageName, vector<vector<int>> rows);
    // void writeMatrixPage(string pageName, vector<vector<int>> rows);
    void deleteFile(string tableName, int pageIndex);
    void deleteFile(string tableName, uint runsThisPass, int pageIndex);
    void deleteFile(string fileName);
    bool copyFile(string sourceFileName, string destinationFileName);
    Page* insertMatrixPageIntoPool(MatrixPage *page);
    void writePage(string tableName, int pageIndex, vector<vector<int>> rows, int rowCount);
    void writePage(string tableName, uint runsThisPass, int pageIndex, vector<vector<int>> rows, int rowCount);
    void updatePageInQueue(Page* page);
    ~BufferManager();
};